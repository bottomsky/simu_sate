...
try {
  if (-not $JustTest.IsPresent) {
    Write-Host "[STEP] Configure CMake for unit tests..." -ForegroundColor Cyan
    & cmake @cmakeArgs
  ...
  if ($Run.IsPresent -or $JustTest.IsPresent) {
    ...
    if (Test-Path $dataDir) {
      Get-ChildItem -Path $dataDir -Filter "multi_simulation_results_step_*.json" | ForEach-Object {
  ...
  Write-Error "[ERROR] Failed to build or run unit tests: $_"
  exit 1
}
...
# - 产出基础测试数据到输出目录（默认 tmp_path，可由 TEST_OUTPUT_DIR 覆盖为持久化目录）。
#
# 运行方式：
...
REPO_ROOT = THIS_FILE.parents[4]  # tests/unit/python/j2_orbit/ -> 上溯 4 层到 simu_sate 项目根
EXAMPLE_DIR = REPO_ROOT / "example"

# 将 example 加入 import 路径，导入 Python 绑定示例
sys.path.insert(0, str(EXAMPLE_DIR))
try:
    from python_binding_example import (  # type: ignore
        J2OrbitPropagator,
        eci_to_ecef_position,
        ecef_to_eci_position,
    )
except SystemExit:
    pytest.skip("示例绑定初始化失败（SystemExit），跳过该目录下的测试", allow_module_level=True)
except Exception as _e:
    pytest.skip(f"示例绑定导入失败：{_e}", allow_module_level=True)


def _save_json(out_dir: Path, name: str, data: dict) -> None:
    # 保存字典为 JSON 文件到 out_dir。
    # 参数: out_dir (Path) 目录；name (str) 文件名（不含路径）；data (dict) 要保存的对象
    # 返回: None
    # 异常: IOError 写入失败时抛出
...
@pytest.mark.order(1)
def test_minimal_end_to_end(ensure_output_subdir):
    # 最小闭环端到端测试。
    # 步骤:
    #   1) 创建传播器并将步长设为 60s；
    #   2) 将初始轨道传播 3600s；
    ...
    #   5) 保存过程数据到输出目录（使用 ensure_output_subdir 管理）。
    # 断言:
    #   - 传播后 t 与目标时间相等；
    #   - 往返后的 a/e/i 与输入近似一致；
    ...
    # 5) 保存过程数据（输出目录策略：默认 tmp_path，可通过 TEST_OUTPUT_DIR 覆盖为持久化目录）
    out_dir = ensure_output_subdir("unit", "j2_orbit", "propagate", "basic_case_001")
    _save_json(
        out_dir,
        "summary.json",
        {

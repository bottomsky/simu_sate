...
ROOT = Path(__file__).resolve().parents[4]  # tests/unit/python/j2_orbit -> 上溯 4 层到 simu_sate 项目根
...
def _save_json(path: os.PathLike | str, data: dict) -> None:
    """保存字典为 JSON 文件（UTF-8）。
    参数:
      - path: 输出文件完整路径
      - data: 待保存的 Python 字典
...
def test_apply_impulse_smoke(dv, expect_equal, ensure_output_subdir):
    """烟囱测试：验证 apply_impulse 的零/非零脉冲行为。
    步骤：
      1. 生成初始根数并传播到 t1
      2. 若符号缺失则 skip
      3. 在 t=0.0 处施加脉冲，比较几何要素等价性（忽略 t）
      4. 持久化对比数据到输出目录（默认 tmp_path，可由 TEST_OUTPUT_DIR 覆盖）
    """
    # 准备初始要素与传播
    ...
    # 保存测试数据（输出目录策略）
    out_dir = ensure_output_subdir("unit", "j2_orbit", "python_binding", "apply_impulse_smoke")
    _save_json(out_dir / f"dv_{'eq' if expect_equal else 'neq'}.json", {"base": base, "after": after, "dv": dv})

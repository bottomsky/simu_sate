...
namespace J2.Propagator
{
    ...
    public sealed class J2ConstellationPropagator : IDisposable
    {
        private IntPtr _handle;

        // P/Invoke methods
        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_create", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr j2_constellation_propagator_create(double epoch_time);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_destroy", CallingConvention = CallingConvention.Cdecl)]
        private static extern void j2_constellation_propagator_destroy(IntPtr handle);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_add_satellite", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_add_satellite(IntPtr handle, ref CCompactOrbitalElements elems);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_add_satellites", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_add_satellites(IntPtr handle, [In] CCompactOrbitalElements[] elems, nuint count);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_is_cuda_available", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_is_cuda_available();

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_propagate", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_propagate(IntPtr handle, double target_time);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_get_satellite_count", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_get_satellite_count(IntPtr handle, out nuint count);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_get_satellite_elements", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_get_satellite_elements(IntPtr handle, nuint satellite_id, out CCompactOrbitalElements elements);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_get_satellite_state", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_get_satellite_state(IntPtr handle, nuint satellite_id, ref CStateVector state);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_get_all_positions", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_get_all_positions(IntPtr handle, [Out] double[] positions, ref nuint count);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_apply_impulse_to_constellation", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_apply_impulse_to_constellation(IntPtr handle, [In] double[] deltaVs, nuint count, double impulse_time);

        // Additional P/Invoke methods for configuration
        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_set_step_size", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_set_step_size(IntPtr handle, double step_size);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_set_compute_mode", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_set_compute_mode(IntPtr handle, ComputeMode mode);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_set_adaptive_step_size", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_set_adaptive_step_size(IntPtr handle, int enable);

        [DllImport(Native.DllName, EntryPoint = "j2_constellation_propagator_set_adaptive_parameters", CallingConvention = CallingConvention.Cdecl)]
        private static extern int j2_constellation_propagator_set_adaptive_parameters(IntPtr handle, double tolerance, double min_step, double max_step);

        // Public API
        public static bool IsCudaAvailable() => j2_constellation_propagator_is_cuda_available() != 0;

        public J2ConstellationPropagator(double epochTimeSeconds)
        {
            _handle = j2_constellation_propagator_create(epochTimeSeconds);
            if (_handle == IntPtr.Zero) throw new PropagationException("Failed to create J2ConstellationPropagator");
        ...
        {
            if (j2_constellation_propagator_add_satellite(_handle, ref elems) != 0)
                throw new PropagationException("Failed to add satellite");
        }

        public void AddSatellites(CCompactOrbitalElements[] elems)
        {
            if (j2_constellation_propagator_add_satellites(_handle, elems, (nuint)elems.Length) != 0)
                throw new PropagationException("Failed to add satellites");
        }

        public void Propagate(double targetTimeSeconds)
        {
            if (j2_constellation_propagator_propagate(_handle, targetTimeSeconds) != 0)
                throw new PropagationException("Failed to propagate constellation");
        }

        public int GetSatelliteCount()
        {
            if (j2_constellation_propagator_get_satellite_count(_handle, out var count) != 0)
        ...
        {
            if (j2_constellation_propagator_get_satellite_elements(_handle, (nuint)index, out var elements) != 0)
        ...
        {
            var state = CStateVector.Create();
            if (j2_constellation_propagator_get_satellite_state(_handle, (nuint)index, ref state) != 0)
        ...
        public double[] GetAllPositions()
        {
            ...
            if (j2_constellation_propagator_get_all_positions(_handle, positions, ref actualCount) != 0)
        ...
        public void ApplyImpulseToConstellation(double[] deltaVs, double targetTimeSeconds)
        {
            ...
            if (j2_constellation_propagator_apply_impulse_to_constellation(_handle, deltaVs, (nuint)satCount, targetTimeSeconds) != 0)
        ...
        {
            if (j2_constellation_propagator_set_step_size(_handle, stepSize) != 0)
                throw new PropagationException("Failed to set step size");
        }

        public void SetComputeMode(ComputeMode mode)
        {
            if (j2_constellation_propagator_set_compute_mode(_handle, mode) != 0)
                throw new PropagationException("Failed to set compute mode");
        }

        public void SetAdaptive(bool enable)
        {
            if (j2_constellation_propagator_set_adaptive_step_size(_handle, enable ? 1 : 0) != 0)
        ...
        {
            if (j2_constellation_propagator_set_adaptive_parameters(_handle, tolerance, minStep, maxStep) != 0)
        ...
        public void Dispose()
        {
            if (_handle != IntPtr.Zero)
            {
                j2_constellation_propagator_destroy(_handle);
                _handle = IntPtr.Zero;
            }
            GC.SuppressFinalize(this);
        }

        ~J2ConstellationPropagator() { Dispose(); }
    }

...
class COrbitalElements(Structure):
    ...
    def __str__(self):
        return (
            f"COrbitalElements(a={self.a:.3f}, e={self.e:.6f}, "
...
class CStateVector(Structure):
    ...
    def __str__(self):
        return (
            f"CStateVector(r=[{self.r[0]:.3f}, {self.r[1]:.3f}, {self.r[2]:.3f}], "
...
# 轨道传播函数
j2_lib.j2_propagator_propagate.argtypes = [
    c_void_p,
    c_double,
...
j2_lib.j2_propagator_elements_to_state.argtypes = [
    c_void_p,
    POINTER(COrbitalElements),
...
j2_lib.j2_propagator_state_to_elements.argtypes = [
    c_void_p,
    POINTER(CStateVector),
    c_double,
    POINTER(COrbitalElements),
...
j2_lib.j2_ecef_to_eci_position.argtypes = [
    POINTER(c_double),
    c_double,
    POINTER(c_double),
]
...
class J2OrbitPropagator:
    ...
    def propagate(self, target_time):
        ...
        ret = j2_lib.j2_propagator_propagate(
            self.handle, target_time, ctypes.byref(result)
        )
        if ret != 0:
    ...
    def elements_to_state(self, elements):
        ...
        state = CStateVector()
        ret = j2_lib.j2_propagator_elements_to_state(
            self.handle, ctypes.byref(c_elements), ctypes.byref(state)
        )
        if ret != 0:
    ...
    def state_to_elements(self, state, time):
        ...
        ret = j2_lib.j2_propagator_state_to_elements(
            self.handle, ctypes.byref(c_state), time, ctypes.byref(elements)
        )
        if ret != 0:
    ...
    def set_adaptive_step_size(self, enable):
        """启用或禁用自适应步长"""
        ret = j2_lib.j2_propagator_set_adaptive_step_size(
            self.handle, 1 if enable else 0
        )
        if ret != 0:
...
def main():
    ...
    try:
        ...
        print(f"\n状态向量:")
        print(
            f"  位置 (m): [{state['r'][0]:.3f}, {state['r'][1]:.3f}, {state['r'][2]:.3f}]"
        )
        print(
            f"  速度 (m/s): [{state['v'][0]:.3f}, {state['v'][1]:.3f}, {state['v'][2]:.3f}]"
        )
        ...
        print(
            f"  ECEF位置: [{ecef_pos[0]:.3f}, {ecef_pos[1]:.3f}, {ecef_pos[2]:.3f}] m"
        )

        # ECEF到ECI转换（验证）
        recovered_eci = ecef_to_eci_position(ecef_pos, utc_time)
        print(
            f"  恢复ECI位置: [{recovered_eci[0]:.3f}, {recovered_eci[1]:.3f}, {recovered_eci[2]:.3f}] m"
        )

...
void benchmarkPropagation(J2ConstellationPropagator& propagator, 
...
void printMemoryUsage(const J2ConstellationPropagator& propagator) {
    size_t sat_count = propagator.getSatelliteCount();
...
int main() {
    ...
    if (cudaAvailable) {
        std::cout << "提示: 创建的 J2ConstellationPropagator 将自动优先使用 CUDA 加速计算" << std::endl;
    } else {
    ...
    for (size_t sat_count : test_sizes) {
        ...
        if (J2ConstellationPropagator::isCudaAvailable()) {
            J2ConstellationPropagator prop_cuda = propagator;
